...
static void test_fbw_write_whole_page(void)
{
	int rc;
	...
	rc = fbw_write(&ctx, write_buf, page_size * 2, true);
	zassert_equal(rc, 0, "expected success");
	...
	/* Write all bytes of a page, verify that next page is not erased */
	rc = fbw_write(&ctx, write_buf, page_size, true);
	zassert_equal(rc, 0, "expected success");
...
static void test_fbw_erase_page(void)
{
	int rc;
	...
	rc = fbw_erase_page(&ctx, FLASH_BASE);
	zassert_equal(rc, 0, "expected success");
...
#ifdef CONFIG_FBW_ERASE
	     ztest_unit_test(test_fbw_write_whole_page),
	     ztest_unit_test(test_fbw_erase_page),
#endif
